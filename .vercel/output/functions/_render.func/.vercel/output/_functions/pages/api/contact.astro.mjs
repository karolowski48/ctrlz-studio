...
function emailTemplate(name, service, message) {
  const serviceLabel = {
  ...
  const displayMessage = message ? message.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>") : "Nie podano";
  return `<!DOCTYPE html>
<html lang="pl">
<head>
  <meta charset="UTF-8">
...
const POST = async ({ request }) => {
  ...
  const name = data.get("name") || "Nieznany";
  ...
  const apiKey = "wklej_tutaj_swoj_klucz_z_resend";
